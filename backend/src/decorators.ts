...
import { InternalError, logger, SystemError } from "@backend/services";
import { CatchErrorParams } from "@backend/types";

const toLogDefault = true;
const toThrowDefault = true;

function CatchError(
  opts: CatchErrorParams = {
    message: "Something went wrong.",
    toLog: toLogDefault,
    toThrow: toThrowDefault,
  },
) {
    ...
    descriptor.value = async (...args) => {
      try {
        ...
        if (opts.toThrow && next) {
          const error =
            e instanceof SystemError ? e : new InternalError(opts.message);
          return next(error);
        }
        throw e;
      }

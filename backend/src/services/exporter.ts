...
import { db } from "@backend/services";
import { Callback } from "@backend/types";
...
async function vouchersToCSV<T extends NodeJS.WritableStream>(
  ...
  let vouchers = await db.repoManager.voucherRepo.find({
    where: { campaignId },
    take,
    skip,
  });
  ...
  while (vouchers.length) {
    ...
    skip += take;
    vouchers = await db.repoManager.voucherRepo.find({
      where: { campaignId },
      take,
      skip,
    });

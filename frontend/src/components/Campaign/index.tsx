...
export default function Campaign({ campaign }: { campaign: ICampaign }) {
  ...
  const [isGenerateVouchersModalOpen, setIsGenerateVouchersModalOpen] = useState(false);
    ...
    <>
      <div className='p-8'>
        ...
        <div className='flex justify-between  p-6 mb-10 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-100 dark:border-gray-300'>
          <div>
            <span className='flex space-x-3 font-normal text-black'>Name: {campaignData.name}</span>
            ...
            {campaignData.vouchers.length > 0 && (
              <CSVLink data={csvVouchers} filename={`${campaign.name} vouchers`}>
                <span className='underline text-sky-500'>Download vouchers</span>
              </CSVLink>
            )}
          </div>
            ...
            <TableComponent columns={columns}>
              {campaignData.vouchers.map((voucher: IVoucher) => (
                <tr
                  ...
                  key={voucher.discountCode}
                >
                  <td className='px-6 py-4'>
                    <button
                      className='underline text-blue-500'
                      onClick={() => {
                        copyToClipboard(voucher.discountCode);
                      }}
                    >
      ...
      />
      {toastMessage && <Toast message={toastMessage} onHideToast={onHideToast} />}
    </>
  );
}
